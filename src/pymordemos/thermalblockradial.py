...
from pymor.analyticalproblems.thermalblockradial import ThermalBlockProblemRadial
...
def thermalblockradial_demo(args):
    args['NRINGE'] = int(args['NRINGE'])
    ...
    print('Setup Problem ...')
    problem = ThermalBlockProblemRadial(num_ringe=(args['NRINGE']))
    ...
    *** RESULTS ***
    ...
    Problem:
       number of blocks:                   {args[NRINGE]}
       h:                                  sqrt(2)/{args[--grid]}

    Greedy basis generation:
       number of snapshots:                {args[SNAPSHOTS]}^({args[NRINGE]})
